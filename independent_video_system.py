...
# -*- coding: utf-8 -*-
"""
Independent Video System - All APIs Working
"""
...
def make_independent_lecture_video(src_path: str, out_dir: str, title: str = None):
    """Create lecture video using all available APIs independently"""
    print("=" * 60)
    print("TAO VIDEO BAI GIANG DOC LAP")
    print("=" * 60)
    ...
    for i, slide_data in enumerate(lecture_data["slides"], 1):
        ...
        slide_clip = create_independent_slide(
            slide_data["script"], 
    ...
    conclusion_clip = create_independent_slide(
    ...
    print(f"\nIndependent video created successfully: {out_video}")
    ...
    return {
        ...
        "lecture_info": {
            ...
            "image_generator": "Independent Multi-API"
        }
    }

def create_independent_slide(script, tmpdir, slide_num, img_path=None):
    """Create independent slide"""
    try:
        # Create slide image
        frame_path = os.path.join(tmpdir, f"independent_slide_{slide_num:03d}.png")
        slide_img = create_slide_image(script, image_path=img_path)
        slide_img.save(frame_path)
        
        # Create audio
        audio_path = os.path.join(tmpdir, f"independent_audio_{slide_num:03d}.mp3")
        print(f"   Synthesizing voice for slide {slide_num}...")
        dur = synthesize_voice_with_elevenlabs(script, audio_path)
...
def test_independent_system():
    """Test independent video system"""
    print("\n" + "=" * 60)
    print("TEST HE THONG VIDEO DOC LAP")
    print("=" * 60)
    ...
    try:
        # Create output directory
        output_dir = "test_independent_final"
        os.makedirs(output_dir, exist_ok=True)
        
        print(f"Created sample document: {temp_file.name}")
        print("Generating independent video with all APIs...")
        
        result = make_independent_lecture_video(
            src_path=temp_file.name,
            out_dir=output_dir,
            title="Independent AI Video Test"
        )
...
def main():
    """Main function"""
    print("HE THONG VIDEO AI DOC LAP")
    print("Su dung TAT CA API keys co san")
    
    # Test independent system
    success = test_independent_system()
    
    if success:
        print("\nHOAN THANH!")
        print("He thong doc lap da hoat dong!")
        print("\nBan co the:")
