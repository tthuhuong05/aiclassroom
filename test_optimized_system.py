...
# -*- coding: utf-8 -*-
"""
Test All APIs and Create Optimized Video System
"""
...
def test_all_api_keys():
    ...
    try:
        ...
        if pexels_key:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get("photos"):
                    print("   OK - Co the tim hinh anh")
                    results["pexels"] = True
                else:
                    print("   OK - Khong tim thay hinh anh")
                    results["pexels"] = True
            else:
    ...
    try:
        ...
        if elevenlabs_key:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get("voices"):
                    print("   OK - Co the tao giong nguoi that")
                    results["elevenlabs"] = True
                else:
                    print("   OK - Khong co voices")
                    results["elevenlabs"] = True
            else:
                print(f"   LOI: {response.status_code}")
    ...
    try:
        ...
        if gemini_key:
            ...
            if response.text:
                print("   OK - Co the phan tich noi dung")
                results["gemini"] = True
            else:
    ...
    try:
        ...
        if stability_key:
            ...
            if response.status_code == 200:
                print("   OK - Co the tao hinh anh bang AI")
...
def create_optimized_video_function():
    """Create optimized video generation function"""
    print("\n" + "=" * 60)
    print("TAO HAM TAI VIDEO TOI UU")
    print("=" * 60)
    
    # Add optimized function to doc_video_service.py
    optimized_function = '''
...
'''
    ...
    # Append to doc_video_service.py
    try:
        with open("services/doc_video_service.py", "a", encoding="utf-8") as f:
            f.write(optimized_function)
        print("OK - Da them ham toi uu vao doc_video_service.py")
        return True
    except Exception as e:
        print(f"LOI - Khong the them ham: {e}")
        return False

def test_optimized_video():
...
def main():
    ...
    create_optimized_video_function()
    
    # Test optimized video generation
    video_ok = test_optimized_video()
    ...
    print(f"APIs hoat dong: {working_apis}")
    print(f"APIs loi: {failed_apis}")
    print(f"Video generation: {'OK' if video_ok else 'FAILED'}")
    
    if video_ok:
        print("\nTHANH CONG!")
        print("He thong da duoc toi uu de su dung TAT CA API keys:")
        ...
        print("\nBan co the su dung ngay trong ung dung web!")
    else:
        print("\nCAN KIEM TRA LAI")
